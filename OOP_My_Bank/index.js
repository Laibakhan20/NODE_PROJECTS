...
class Customer {
    ...
    mobileNumber;
    accountNumber;
    constructor(fname, lname, age, gender, mmobnumber, accnumber) {
...
async function bankService(bank) {
    do {
        ...
        if (service.select == "View Account Balance") {
            ...
            let account = myBank.accounts.find((acc) => acc.accountNumber == res.accNum);
            if (!account) {
                console.log("Invalid account number. Account not found");
            }
            if (account) {
                //making function to find customer:
                let name = myBank.customers.find((item) => item.accountNumber == account?.accountNumber); //?:existornot
                console.log(`Dear ${chalk.bold.blueBright(name?.firstName)} ${chalk.blue.bold(name?.lastName)}, Your account balance is: ${chalk.green(`$${account.balance}`)}`);
            }
        }
        //for cash withdraw:
